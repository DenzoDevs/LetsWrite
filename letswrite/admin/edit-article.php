...
$query=mysqli_query($con,"update articles set PostTitle='$posttitle',CategoryId='$catid',SubCategoryId='$subcatid',PostDetails='$postdetails',PostUrl='$url',Is_Active='$status',lastUpdatedBy='$lastuptdby' where id='$postid'");
if($query)
{
$msg="Post updated ";
}
...
<html lang="en">
    <head>
        ...
        <!-- App title -->
        <title>Lets Write | Add Post</title>
...
<?php
$postid=intval($_GET['pid']);
$query=mysqli_query($con,"select articles.id as postid,articles.PostImage,articles.PostTitle as title,articles.PostDetails,category.CategoryName as category,category.id as catid,subcategory.SubCategoryId as subcatid,subcategory.Subcategory as subcategory from articles left join category on category.id=articles.CategoryId left join subcategory on subcategory.SubCategoryId=articles.SubCategoryId where articles.id='$postid' and articles.Is_Active=1 ");
