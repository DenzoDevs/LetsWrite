...
if($query):
 ...
 echo "<script>alert('Something went wrong. Please try again.');</script>";  
    ...
    $sql = "SELECT viewCounter FROM articles WHERE id = '$postid'";
    $result = $con->query($sql);

    if ($result->num_rows > 0) {
        while($row = $result->fetch_assoc()) {
            $visits = $row["viewCounter"];
            $sql = "UPDATE articles SET viewCounter = $visits+1 WHERE id ='$postid'";
    $con->query($sql);
...
<html lang="en">
  ...
  <head>
    ...
    <title>Lets Write| Home Page</title>
...
$currenturl="http://".$_SERVER['HTTP_HOST'] . $_SERVER['REQUEST_URI'];;
 $query=mysqli_query($con,"select articles.PostTitle as posttitle,articles.PostImage,category.CategoryName as category,category.id as cid,subcategory.Subcategory as subcategory,articles.PostDetails as postdetails,articles.PostingDate as postingdate,articles.PostUrl as url,articles.postedBy,articles.lastUpdatedBy,articles.UpdationDate from articles left join category on category.id=articles.CategoryId left join  subcategory on  subcategory.SubCategoryId=articles.SubCategoryId where articles.id='$postid'");
...
<input type="text" name="name" class="form-control" placeholder="Enter your fullname" required>
 ...
 <?php 
 $sts=1;
 $query=mysqli_query($con,"select name,comment,postingDate from  comments where postId='$postid' and status='$sts'");
while ($row=mysqli_fetch_array($query)) {
?>
